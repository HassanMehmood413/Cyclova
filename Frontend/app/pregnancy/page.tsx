...
export default function PregnancyTracker() {
  ...
  const fetchInsights = async () => {
    try {
      ...
      if (!setupData.last_period) {
        setupData.last_period = new Date().toISOString().split('T')[0];
        console.warn('Missing last period for insights, using current date:', setupData.last_period);
        return;
      }

      if (!symptomData.symptom) {
        symptomData.symptom = 'Unknown';
        console.warn('Missing symptom for insights, using "Unknown":', symptomData.symptom);
        return;
      }

      // Log the data we're sending for debugging
      console.log('Sending insight request with data:', {
        setup_data: {
          last_period: setupData.last_period || null,
          known_due_date: setupData.known_due_date || null,
          first_pregnancy: setupData.first_pregnancy,
          pre_pregnancy_weight: setupData.pre_pregnancy_weight ? parseFloat(setupData.pre_pregnancy_weight) : 0,
          height: setupData.height ? parseFloat(setupData.height) : 0
        },
        symptom_data: {
      ...
      const requestBody = {
        setup_data: {
          last_period: setupData.last_period || null,
          known_due_date: setupData.known_due_date || null,
          first_pregnancy: Boolean(setupData.first_pregnancy),
          pre_pregnancy_weight: setupData.pre_pregnancy_weight ? parseFloat(setupData.pre_pregnancy_weight) : 0,
          height: setupData.height ? parseFloat(setupData.height) : 0
        },
        symptom_data: {
      ...
      setInsights(data.insights); // Access the insights field from the response
    } catch (err) {
        ...
        <div className="tracker-content">
          ...
          {pregnancyData && (
            <div className="insights-tab">
              <button onClick={fetchInsights}>AI Insights</button>
              {insights ? (
